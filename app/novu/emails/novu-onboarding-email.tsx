...
import { Body, Button, CodeInline, Column, Container, Head, Heading, Html, Img, Preview, render, Row, Section, Tailwind, Text } from '@react-email/components';

import { ControlSchema, PayloadSchema } from '../workflows';

type NovuWelcomeEmailProps = ControlSchema & PayloadSchema;

export const NovuWelcomeEmail = ({ components, userImage, teamImage, arrowImage, showHeader }: NovuWelcomeEmailProps) => {
  return (
    <Html>
      <Head />
      ...
      >
        <Body className="bg-blurwhite text-base font-sans">
                ...
                <Section key={componentIndex}>
                  ...
                  {component.type === 'users' ? (
                    <Section className={'mb-5'}>
                      <Row>
                        <Text className={`text-[#666666] text-[12px] leading-[24px] text-${component.align}`}>{component.text}</Text>
                      </Row>
          ...
          <Container className="mt-20">
            <Text className="text-center text-gray-400 mb-45">Powered by Novu, the Code-First Notification Infrastructure</Text>
          </Container>
        </Body>
      </Tailwind>
    </Html>
